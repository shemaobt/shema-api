...
async def test_the_team_approves_a_passage_whose_conversation_fell_short(client, db_session):
    ...
    The floor unmet is a fact of the ledger and no scene practised is nothing the packet
    states any more (v0.7), and neither is a door: a team that rehearsed through the record
    entry and told the passage back is answered with a version, not halted for a person
    nobody can help.
    """
    project, credential = await a_claimed_device(db_session)
    session = await ready_session(db_session, project_id=project.id)
    ...
    assert "comprehension" not in row.packet
...
async def test_the_desk_reads_that_passage_with_no_blocker(client, db_session, room_app):
    ...
    assert "comprehension" not in read.json()
