...
The second half is the other side of the same guard, read from the live turn: what the team
hears when the assessor call itself fails, and how a room degrades when it keeps failing.
"""
...
class _ScriptedAssessor:
    ...
    the script runs out the assessor works: a call the test did not plan for can only make
...
async def test_one_failure_between_good_turns_never_reaches_the_hard_stop(
    ...
    assert all(line not in hard_stop for line in spoken)
