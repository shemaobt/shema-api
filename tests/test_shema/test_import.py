...
# --- dates are converted at the boundary --------------------------------------------------
def test_the_export_writes_day_first() -> None:
    assert to_export_date("13/04/2024", "x") == (date(2024, 4, 13), None)
    assert to_export_date("01/03/2022", "x") == (date(2022, 3, 1), None)


def test_an_empty_date_is_null_and_not_a_refusal() -> None:
    assert to_export_date("", "x") == (None, None)


def test_a_string_that_is_not_an_export_date_stops_the_run() -> None:
...
def test_a_day_no_calendar_has_stops_the_run() -> None:
...
def test_a_month_above_twelve_stops_the_run() -> None:
    assert to_export_date("13/13/2024", "x")[1] is not None


# --- the flag is the client's answer ------------------------------------------------------
def test_with_no_list_every_record_is_sensitive() -> None:
...
def test_a_confirmed_country_clears_the_record() -> None:
    built = plan([export_row()], cleared("South Africa"))
    assert only_record(built).flag.sensitive is False


def test_a_country_the_list_does_not_name_is_sensitive() -> None:
...
def test_a_country_the_client_confirmed_sensitive_is_sensitive() -> None:
...
def test_one_unanswered_country_flags_the_whole_record() -> None:
...
def test_an_empty_location_is_sensitive() -> None:
    built = plan([export_row(location="", coords=[0, 0])], cleared("South Africa"))
    assert only_record(built).flag.sensitive is True


def test_the_export_may_raise_the_flag_over_a_cleared_country() -> None:
...
def test_an_unrecognised_sensitivity_word_raises_the_flag() -> None:
    built = plan([export_row(sensitivity="Restricted-ish")], cleared("South Africa"))
    assert only_record(built).flag.sensitive is True


def test_unrestricted_is_not_a_reason_to_flag() -> None:
    built = plan([export_row(sensitivity="Unrestricted")], cleared("South Africa"))
    assert only_record(built).flag.sensitive is False


def test_a_verdict_the_file_cannot_state_is_refused(tmp_path: Path) -> None:
...
def test_a_file_without_a_countries_object_is_refused(tmp_path: Path) -> None:
    path = tmp_path / "countries.json"
...
def test_a_list_that_answers_is_not_pending(tmp_path: Path) -> None:
    path = tmp_path / "countries.json"
    path.write_text(
        json.dumps(
            {
...
# --- nothing is cleaned in silence --------------------------------------------------------
def test_a_record_whose_keys_are_not_the_exports_stops_the_run() -> None:
    row = export_row()
    row["somethingNew"] = ""
...
def test_a_missing_key_stops_the_run() -> None:
    row = export_row()
    del row["notes"]
    built = plan([row], SensitiveCountries.pending())
    assert built.records == []
...
def test_a_duplicate_id_stops_the_run() -> None:
    built = plan([export_row(), export_row()], SensitiveCountries.pending())
    assert len(built.records) == 1
    assert "more than once" in built.refusals[0].detail


def test_a_child_collection_with_rows_stops_the_run_rather_than_being_dropped() -> None:
...
def test_the_ambiguous_values_are_reported_and_still_imported() -> None:
    built = plan(
        [
            export_row(
                id="jaminawa",
...
def test_a_disagreement_with_the_exports_own_flag_is_reported() -> None:
...
def test_the_columns_empty_on_every_record_are_counted() -> None:
...
def test_answered_no_and_answered_none_are_not_counted_as_empty() -> None:
...
def test_the_pending_list_does_not_report_the_disagreement_on_every_record() -> None:
...
def test_a_language_name_keeps_its_non_breaking_space() -> None:
...
def test_a_language_code_is_checked_and_never_refused() -> None:
...
def test_the_export_row_is_kept_beside_the_parsed_values() -> None:
...
def test_an_unassessed_health_dimension_is_null_and_not_boa() -> None:
...
def test_the_approved_count_is_imported_as_it_stands_and_marked_unverified() -> None:
    record = only_record(
...
# --- the run is idempotent and dry-runnable -----------------------------------------------
async def test_a_dry_run_writes_nothing(db_session: AsyncSession) -> None:
    outcome = await _import(db_session, [export_row()], SensitiveCountries.pending(), write=False)
    assert len(outcome.inserted) == 1
    assert (await db_session.execute(select(ShemaProject))).scalars().all() == []


async def test_the_slug_is_the_id_and_no_new_one_is_minted(db_session: AsyncSession) -> None:
...
async def test_a_sensitivity_typed_over_the_imported_one_holds_the_flag(
    db_session: AsyncSession,
) -> None:
    """The flag keeps no provenance, so the text beside it is where a person shows."""
    rows = [export_row()]
    await _import(db_session, rows, SensitiveCountries.pending())
    stored = await _stored(db_session, "afrikaans-kaaps")
    stored.sensitivity = "Confidential"
    await db_session.commit()

    outcome = await _import(db_session, rows, cleared("South Africa"), allow_lowering=True)
    assert len(outcome.flags_lowering_refused) == 1
    assert outcome.flags_lowered == []
    assert (await _stored(db_session, "afrikaans-kaaps")).sensitive_country is True


async def test_a_sensitivity_nobody_typed_over_does_not_hold_the_flag(
    db_session: AsyncSession,
) -> None:
    """The question is *edited since the import*, not *says something*.

    The export's own ``Confidential`` already holds the flag up through ``_is_sensitive``,
    and a record nobody has typed over is one this program may still lower — otherwise the
    127 placeholders the client's list clears would never come off.
    """
    rows = [export_row(sensitivity="Unrestricted")]
    await _import(db_session, rows, SensitiveCountries.pending())

    outcome = await _import(db_session, rows, cleared("South Africa"), allow_lowering=True)
    assert outcome.flags_lowering_refused == []
    assert len(outcome.flags_lowered) == 1
    assert (await _stored(db_session, "afrikaans-kaaps")).sensitive_country is False


async def test_a_flag_is_raised_without_asking_for_permission(db_session: AsyncSession) -> None:
...
async def test_a_record_born_in_the_product_is_never_touched(db_session: AsyncSession) -> None:
    db_session.add(
        ShemaProject(
            id="afrikaans-kaaps",
