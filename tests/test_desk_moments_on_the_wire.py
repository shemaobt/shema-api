...
async def a_room_facilitator(db: AsyncSession, room_app) -> dict[str, str]:
...
async def test_the_inbox_says_when_the_hand_went_up(room_client, db_session, room_app):
    ...
    headers = await a_room_facilitator(db_session, room_app)
    session = IRSession(id="sessao-da-pergunta", pericope="P03")
...
async def test_a_take_says_when_it_was_recorded(room_client, db_session, room_app):
    headers = await a_room_facilitator(db_session, room_app)
    session = IRSession(id="sessao-do-fuso", pericope="P03")
    db_session.add(session)
    await db_session.commit()
